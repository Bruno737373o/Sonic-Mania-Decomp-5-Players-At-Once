...
public class RSDK extends GameActivity {
    static {
        // Carrega as bibliotecas nativas do motor
    ...
    public String[] fsDirIter(byte[] path) {
        ...
        if (dir == null || dir.isFile()) return new String[0];
        List<String> out = new ArrayList<String>();
        for (DocumentFile file : dir.listFiles()) {
            if (file.isDirectory()) out.add(new String(path) + "/" + file.getName());
        }
    ...
    static class RecursiveIterator {
        ...
        public RecursiveIterator(@NonNull ContentResolver resolver, String path, Uri uri) {
            ...
            this.docs.add(resolver.query(DocumentsContract.buildChildDocumentsUriUsingTree(base, DocumentsContract.getDocumentId(base)), 
                new String[]{Document.COLUMN_DOCUMENT_ID, Document.COLUMN_MIME_TYPE}, null, null, null));
        }

        public static RecursiveIterator get(ContentResolver resolver, byte[] path, Uri uri) {
            String sPath = new String(path);
            if (iterators.get(sPath) != null) return iterators.get(sPath);
            RecursiveIterator iter = new RecursiveIterator(resolver, sPath, uri);
            iterators.put(sPath, iter);
            return iter;
        }
        ...
        public String next() {
            try {
                ...
                if (c.getString(1).equals(Document.MIME_TYPE_DIR)) {
                    docs.push(resolver.query(DocumentsContract.buildChildDocumentsUriUsingTree(uri, DocumentsContract.getDocumentId(uri)), 
                        new String[]{Document.COLUMN_DOCUMENT_ID, Document.COLUMN_MIME_TYPE}, null, null, null));
                    return next();
                }
    ...
    public void showLoadingIcon() { loadingIcon.show(); }
    public void hideLoadingIcon() { loadingIcon.hide(); }
}
